...
### URL para crear las credenciales

👉 **https://www.reddit.com/prefs/apps**

(Debes estar iniciado sesión en Reddit.)

### Qué hay que hacer (paso a paso)

1. Abre **[https://www.reddit.com/prefs/apps](https://www.reddit.com/prefs/apps)** en el navegador.
2. Baja hasta el final de la página y pulsa:
   - **“are you a developer? create an app…”**, o
   - **“Create App”** / **“Create Another App”**
3. Rellena el formulario exactamente así:

   | Campo | Qué poner |
   |-------|-----------|
   | **name** | `explorador-de-redit` (o cualquier nombre) |
   | **tipo de app** | Selecciona **script** |
   | **description** | Opcional (puedes dejarlo vacío) |
   | **about url** | Déjalo vacío |
   | **redirect uri** | `http://localhost:8080` |

4. Pulsa el botón **“create app”**.
5. En la tarjeta de la aplicación que acaba de crearse verás:
   - **Client ID**: la cadena corta que aparece **justo debajo del nombre** de la app.
   - **Client Secret**: la línea etiquetada como **“secret”**.

6. Copia esos dos valores:
   - En la **interfaz web**: pégalos en los campos *Client ID* y *Client Secret* (modo “Con credenciales”).
   - En el **CLI**: pégalos en el archivo `.env` (`REDDIT_CLIENT_ID` y `REDDIT_CLIENT_SECRET`).

> **Importante:** no compartas el *client_secret* públicamente. El archivo `.env` está en el `.gitignore` para que no se suba a GitHub. En la web las credenciales solo se usan en tu máquina (y solo se guardan en el navegador si marcas “Recordar”).
...
4. Si elegiste *Con credenciales*:
   - Sigue las instrucciones que aparecen en pantalla (o la sección [Obtener credenciales](#obtener-credenciales-de-reddit-opcional-pero-recomendado)).
...
1. Crea las credenciales en **https://www.reddit.com/prefs/apps** (ver sección anterior).
...
- Para guardar un hilo importante o con muchos comentarios → *Con credenciales* (crea la app en https://www.reddit.com/prefs/apps)
